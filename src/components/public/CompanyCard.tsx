...
export function CompanyCard({ company }: CompanyCardProps) {
  ...
  return (
    <Link href={`/empresas/${company.slug}`} className="group block">
      <div className="relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-card transition-all duration-300 hover:-translate-y-1.5 hover:border-blue-200/80 hover:shadow-premium-lg">
        ...
        <div className="relative h-28 w-full overflow-hidden bg-gradient-to-br from-blue-600 via-blue-500 to-sky-500">
          ...
          {company.isFeatured && (
            <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-blue-700 shadow-sm backdrop-blur">
              <Star className="h-3 w-3 fill-blue-500 text-blue-500" /> Destacada
            </span>
          )}

          {/* Flecha que aparece al hover */}
          <span className="absolute left-3 top-3 flex h-8 w-8 translate-y-1 items-center justify-center rounded-full bg-white/90 text-blue-600 opacity-0 shadow-sm backdrop-blur transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
        ...
        <div className="-mt-9 flex justify-center px-4">
          {company.logoUrl ? (
            <div className="relative h-[4.5rem] w-[4.5rem] overflow-hidden rounded-2xl border-4 border-white bg-white shadow-premium">
        ...
        <div className="flex flex-1 flex-col px-5 pb-5 pt-3 text-center">
          <h3 className="line-clamp-1 font-semibold tracking-tight text-slate-900 transition-colors group-hover:text-blue-700">
            {company.name}
          </h3>
          ...
          <div className="mt-auto flex items-center justify-center gap-4 border-t border-slate-100 pt-4 text-xs text-slate-600">
            <span className="inline-flex items-center gap-1">
