...
| **Automation Playbooks** | ✅ E1.1–E1.7 | `src/lib/automation/playbooks` — estrategias comerciales instalables (24 apartados del Documento Maestro) que envuelven automatizaciones reales de E1. E1.1: 20 captación (`ACQ-*`). E1.2: 18 onboarding (`ONB-*`). E1.3: 12 primera compra (`FP-*`). E1.4: 16 frecuencia (`FREQ-*`). E1.5: 16 recuperación (`REC-*`). E1.6: 20 membresías (`MEM-*`, +`compatibleMembershipModels`). E1.7: 20 referidos (`REF-*`, journey completo sobre el Referral Engine, +`compatibleReferralModels`). 122 playbooks universales y editables. Sin migración: reutiliza las tablas de E1. Próximas: E1.8–E1.10. |
